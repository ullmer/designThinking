...
class enShapefile:
  ...
  def calcNormLatLong(self, lat, long):
    ...
    if lat < self.latMin or lat > self.latMax:
      print("enShapefile:calcNormLatLong: lat arg outside range"); return [0,0]

    if long < self.longMin or long > self.longMax:
      print("enShapefile:calcNormLatLong: long arg outside range"); return [0,0]
  ...
  def cairoCreatePngSurface(self): #use Cairo to create surface for plotting
    ...
    self.ctx.scale(self.pixelScale*100, self.pixelScale*100)
  ...
  def drawCircle(self, vert, diam): 
    if vert == None: print("drawCircle called with vert==None"); return 
    ...
    try:    cr.translate(vert[0], vert[1])
