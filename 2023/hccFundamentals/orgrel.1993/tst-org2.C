...
main(int argc, char **argv)
{
  ...
  if (argc >= 4) { unsigned int n = atoi(argv[3]);
...
void readData(int numPeople, char *filename)
{
  ...
  VpfGravity *peopleRepulsion = new VpfGravity;
  peopleRepulsion->setConst(-0.1); //Antigravity
  peopleRepulsion->setPower(1.1);
...
//  char **names = new (char **)[numPeople];
  ...
  VpfGravity *groupRepulsion = new VpfGravity;
  groupRepulsion->setConst(-0.1); //Antigravity
